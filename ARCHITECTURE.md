...
| *(generated)*           | `"geom"`                   | geometry        | Generated by DB from Lat/Lng — **not inserted**|
...
> **Note:** `"geom"` is omitted — it is a generated column; PostgreSQL computes it
> automatically from `"Latitude"` and `"Longitude"`. Explicitly inserting a value raises
> `ERROR: cannot insert a non-DEFAULT value into column "geom"`.
...
It is a **generated column** — the CrashMap schema defines it as:

```sql
GENERATED ALWAYS AS (ST_SetSRID(ST_MakePoint("Longitude", "Latitude"), 4326)) STORED
```

PostgreSQL computes `"geom"` automatically whenever `"Latitude"` and `"Longitude"` are inserted
or updated. **Do not include `"geom"` in INSERT statements** — doing so raises
`ERROR: cannot insert a non-DEFAULT value into column "geom"`.
...
INSERT INTO crashdata (
  ...
  ('3887523', 'City Street', 'Washington', NULL, 'King', 'Seattle',
   ...
   NULL, 2, 47.668680009423, -122.376289485757, 'Pedestrian')
ON CONFLICT ("ColliRptNum") DO NOTHING;
```
...
- Implement field mapping: NULL coercion, `'` sanitization, string escaping, `CrashDate`
  derivation (`"geom"` is a generated column — excluded from INSERT)
...
| Bad coordinates cause `"geom"` generation failure | Low | Medium | Validate lat/long ranges before generating SQL; skip records with `NULL` coordinates |
